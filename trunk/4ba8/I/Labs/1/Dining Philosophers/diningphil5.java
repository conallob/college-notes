...
public class MyNode extends Node implements GroupNode{
   ...
   public MyNode(){
    ...
    public void receiveMessage(Node sender, Message message) {
	    ...
	    if(message.isType("leftfork"))
		{
			Message response = new Message(sender, "response");
			if(getBoolean("left") || getBoolean("eating") && getID() > sender.getID()) //have it
		 	{
		...
		{
			Message response = new Message(sender, "response");
			if(getBoolean("right") || getBoolean("eating") && getID() > sender.getID()) //have it
		 	{
		 		response.putString("fork", "Taken");
		 	}else{ // don't have it
